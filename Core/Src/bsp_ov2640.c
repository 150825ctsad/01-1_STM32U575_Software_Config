...
uint8_t OV2640_Init(void)
{
  uint16_t i=0;
	...
	i2c_sent(OV2640_DSP_RA_DLMT, 0x01);	//操作sensor寄存器
 	i2c_sent(OV2640_SENSOR_COM7, 0x80);	//软复位OV2640

	reg=i2c_receive(OV2640_SENSOR_MIDH);	//读取厂家ID 高八位
	reg<<=8;
	reg|=i2c_receive(OV2640_SENSOR_MIDL);	//读取厂家ID 低八位
	if(reg!=OV2640_MID)
	{
		printf("MID:%d\r\n",reg);
		return 1;
	}
	reg=i2c_receive(OV2640_SENSOR_PIDH);	//读取厂家ID 高八位
	reg<<=8;
	reg|=i2c_receive(OV2640_SENSOR_PIDL);	//读取厂家ID 低八位
	if(reg!=OV2640_PID)
	{
		printf("HID:%d\r\n",reg);
		return 2;
	...
	{
	   	i2c_sent(ov2640_sxga_init_reg_tbl[i][0],ov2640_sxga_init_reg_tbl[i][1]);
 	} 
...
void OV2640_JPEG_Mode(void) 
{
	...
	{
		i2c_sent(ov2640_yuv422_reg_tbl[i][0],ov2640_yuv422_reg_tbl[i][1]); 
	} 
	
	//设置:输出JPEG数据
	for(i=0;i<(sizeof(ov2640_jpeg_reg_tbl)/2);i++)
	{
		i2c_sent(ov2640_jpeg_reg_tbl[i][0],ov2640_jpeg_reg_tbl[i][1]);  
	}  
}
//OV2640切换为RGB565模式
void OV2640_RGB565_Mode(void) 
{
	...
	{
		i2c_sent(ov2640_rgb565_reg_tbl[i][0],ov2640_rgb565_reg_tbl[i][1]); 
	} 
} 
//自动曝光设置参数表,支持5个等级
...
void OV2640_Light_Mode(uint8_t mode)
{
	...
	i2c_sent(0XFF,0X00);	 
	i2c_sent(0XC7,0X40);	//AWB OFF 
	i2c_sent(0XCC,regccval); 
	i2c_sent(0XCD,regcdval); 
	i2c_sent(0XCE,regceval);  
}
//色度设置
//0:-2
//1:-1
...
void OV2640_Contrast(uint8_t contrast)
{
	...
	i2c_sent(0x7c,0x07);
	i2c_sent(0x7d,0x20);
	i2c_sent(0x7d,reg7d0val);
	i2c_sent(0x7d,reg7d1val);
	i2c_sent(0x7d,0x06);
}
//特效设置
//0:普通模式    
